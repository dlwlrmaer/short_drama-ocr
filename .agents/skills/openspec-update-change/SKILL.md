...
`$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 是一个可选工作流，可能未安装。在下方任何地方建议它之前，请验证它是否可用。若不可用，`openspec-cn status --change "<name>" --json` 显示下一个制品，`openspec-cn instructions "<artifact-id>" --change "<name>" --json` 解释如何创建它。
...
1. **选择变更**
   ...
   始终宣告："使用变更：<name>"，以及如何覆盖（例如 `$openspec-update-change (Codex) or /openspec-update-change (other agents) <other>`）。
...
4. **读取并调和**
   ...
   - 记录所有现在不一致、缺失或矛盾的内容。
   - 仅修订已存在的文件（`existingOutputPaths`）。不要创建尚不存在的制品，且不要在 glob 制品下创建新文件 — 指出它们并引导用户使用 `$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 来创建。
   - 若变更已一致，说明情况且不做编辑。
...
6. **指出下一步（仅供参考 - 绝不要执行）**
   - 制品仍缺失 -> 建议 `$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 来创建它们。
   - 变更已实现（任务已勾选 / 已 apply） -> 代码可能不再匹配修订后的计划；建议 `$openspec-apply-change (Codex) or /openspec-apply-change (other agents)` 将增量带入代码。
   - 一切完成且已实现 -> 建议 `$openspec-archive-change (Codex) or /openspec-archive-change (other agents)`。

**输出**

每次调用后，展示：
- 修订了哪些制品（以及哪些提议的修订被拒绝）
- 推迟到 `$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 的任何内容（尚未创建的制品或文件）
- 变更的状态及推荐的下一步命令

**护栏**
- 仅规划制品 — 绝不要编辑实现代码。若修订后的计划暗示代码更改，停止并指向 `$openspec-apply-change (Codex) or /openspec-apply-change (other agents)`。
- 使用 `openspec-cn status` 报告的制品 ID 和路径；绝不要基于硬编码的制品名称分支。
- 仅编辑 `existingOutputPaths` 中的具体文件；绝不要写入 glob `resolvedOutputPath`。
- 不要推进构建边界：不创建新制品，不在 glob 制品下创建新文件 — 那是 `$openspec-continue-change (Codex) or /openspec-continue-change (other agents)` 的职责。
- 在写入前与用户确认每个编辑。
- 若请求更改的是变更的*意图*而非细化，首先验证可选 `$openspec-new-change (Codex) or /openspec-new-change (other agents)` 工作流是否可用。若可用，建议用 `$openspec-new-change (Codex) or /openspec-new-change (other agents)` 重新开始（"更新 vs 重新开始" 启发式）。若不可用，请求一个不同的未使用变更名称并建议改用 `openspec-cn new change "<new-change-name>"`。
