...
**Input**: 可选地指定变更名称（例如 `$openspec-apply-change (Codex) or /openspec-apply-change (other agents) add-auth`）。若省略，检查能否从对话上下文推断。若模糊或歧义，你必须提示用户从可用变更中选择。
...
1. **选择变更**
   ...
   始终宣告："使用变更：<name>"，以及如何覆盖（例如 `$openspec-apply-change (Codex) or /openspec-apply-change (other agents) <other>`）。
...
3. **获取实现指令**
   ...
   **处理状态：**
   - 若 `state: "blocked"`（缺少制品）：显示消息，建议使用 `$openspec-continue-change (Codex) or /openspec-continue-change (other agents)`（若未安装，运行 `openspec-cn status --change "<name>" --json` 查看下一个制品，`openspec-cn instructions <artifact-id> --change "<name>" --json` 了解如何创建）
...
所有任务完成！你可以用 `$openspec-archive-change (Codex) or /openspec-archive-change (other agents)` 归档此变更。
```
